'''使用TensorFlow进行手写数字识别的任务。首先加载MNIST数据集，并对训练集和测试集的图片数据进行归一化处理。
然后定义了一个包含两个全连接层的神经网络模型，使用Adam优化器和稀疏分类交叉熵作为损失函数进行训练。
在训练过程中，通过断点续训和保存模型的检查点来保留模型的最佳权重。
最后，通过可视化训练过程中的准确率和损失曲线来评估模型的性能。'''
